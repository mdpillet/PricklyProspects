...
relPath <- "F:/Chapter1/"
diversityPath <- "Data/DiversityMaps/RawReanalysis/"
...
for (i in 1:ncell(aggrStack)) {
  ...
  if (length(skipVector) > 1) {
    ...
    if (length(unique(tmp$ProjectionDistance)) > 1) {
      print(i)
      anovList[i] <- summary(aov(tmp[, i] ~ Time + RCP + GCM + VariableSelection + Model + ProjectionDistance + SamplingDistance + CorrelationFilter, data = tmp))    
    }
    else anovList[i] <- NA
  }
  else anovList[i] <- NA
...
# Export ANOVAs
saveRDS(anovList, paste0(relPath, uncPath, "ANOVA_reanalysis.rds"))
anovList <- readRDS(paste0(relPath, uncPath, "ANOVA_reanalysis.rds"))

# Calculate uncertainties
uncComponents <- data.frame(Driver = c("Time", "RCP", "GCM", "VariableSelection", "Model", "ProjectionDistance", "SamplingDistance", "CorrelationFilter"))
for (i in 1:length(anovList)) {
  print(i)
  if (is.na(anovList[i])) uncComponents[, i+1] <- NA
  else uncComponents[i+1] <- (anovList[[i]]$`Sum Sq` / sum(anovList[[i]]$`Sum Sq`) * 100)[1:8]
}
...
writeRaster(uncMap, paste0(relPath, uncPath, "uncertaintyMap_reanalysis.tif"), overwrite = T)
uncMap <- raster(paste0(relPath, uncPath, "uncertaintyMap_reanalysis.tif"))
...
legend("topright", legend = c("CorrelationFilter", "Model", "ProjectionDistance", "SamplingDistance", "VariableSelection"),
       fill = rainbow(5))
uncPlot <- gplot(uncMap) + geom_tile(aes(fill = factor(value,
  ...
  scale_fill_discrete(name = "Driver", labels = c("CorrelationFilter", "Model", "ProjectionDistance", "SamplingDistance", "VariableSelection"), na.value = NA)
ggsave(paste0(relPath, figPath, "topUncertaintySource_reanalysis.png"))
