...
outPath <- "Data/DiversityMaps/SummaryReanalysis/"

# Set raster options
rasterOptions(maxmemory = 25e+09)
rasterOptions(chunksize = 25e+09)
...
# Split maps by dispersal
futured0 <- future70[grepl("d0", future70)]
futured100 <- future70[grepl("d100", future70)]
futured500 <- future70[grepl("d500", future70)]
d0Stack <- stack(paste0(relPath, diversityPath, futured0))
d100Stack <- stack(paste0(relPath, diversityPath, futured100))
d500Stack <- stack(paste0(relPath, diversityPath, futured500))

# Calculate mean richness maps
beginCluster(detectCores() - 1)
d0Mean <- clusterR(d0Stack, calc, args = list(mean, na.rm = T))
writeRaster(d0Mean, paste0(relPath, outPath, "mean_d0.tif"), overwrite = T)
d100Mean <- clusterR(d100Stack, calc, args = list(mean, na.rm = T))
writeRaster(d100Mean, paste0(relPath, outPath, "mean_d100.tif"), overwrite = T)
d500Mean <- clusterR(d500Stack, calc, args = list(mean, na.rm = T))
writeRaster(d500Mean, paste0(relPath, outPath, "mean_d500.tif"), overwrite = T)
endCluster()